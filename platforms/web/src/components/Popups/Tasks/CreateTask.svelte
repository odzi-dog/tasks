<script lang="ts">
  ...
  // Importing queries
  import { CreateTaskMutation } from '$lib/modules/Tasks/queries';
  import type { ICreateTaskMutationResult } from '$lib/modules/Tasks/queries';
  ...
  function create() {
    ...
    if (!collectionId || taskId) {
      ErrorHandler.throw({
        ...
        message: 'You need to select collection or task'
      })
    } else {
      // Creating new task
      client.request<ICreateTaskMutationResult>(CreateTaskMutation, {
        collectionId,
        taskId,
        input: {
          icon,
...
<div class="w-full h-full flex flex-col rounded-bg bg-zinc-800 rounded-xl">
  ...
  <div class="w-full flex items-center justify-between rounded-t-xl bg-zinc-700 py-2 px-4">
    <!-- Title -->
    <div>
      <h1 class="opacity-70 text-md text-white">Создать задание</h1>
    </div>
